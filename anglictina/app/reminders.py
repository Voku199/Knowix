...
# Přidej tyto endpointy do reminders.py

@reminders_bp.route('/push/subscribe', methods=['POST'])
def push_subscribe():
    """Uložení push subscription"""
    if 'user_id' not in session:
        return jsonify({'success': False, 'error': 'Nepřihlášen.'}), 401
    ...
    try:
        ...
        if not subscription:
            return jsonify({'success': False, 'error': 'Chybí subscription.'}), 400

        conn = get_db_connection()
        cur = conn.cursor()

        # Zkontroluj jestli subscription už existuje
        cur.execute(
        ...
        if existing:
            ...
            cur.execute("""
                ...
                SET p256dh = %s, auth = %s, installed = %s, last_seen = NOW() 
                WHERE endpoint = %s
            """, (
            ...
            # Insert new
            cur.execute("""
                INSERT INTO push_subscriptions (user_id, endpoint, p256dh, auth, installed, created_at, last_seen)
        ...
        print(f"[push] Subscribe error: {ex}")
        return jsonify({'success': False, 'error': 'Chyba serveru.'}), 500
    finally:
...
@reminders_bp.route('/push/vapid-public-key')
def vapid_public_key():
    """Vrátí VAPID public key pro frontend"""
    return jsonify({
        'success': True,
