"""Bezpečnostní rozšíření: rate limiting (Flask-Limiter) + vlastní CSRF ochrana.

init_security(app) zapojí:
  - Limiter: Redis storage v produkci, in-memory v dev nebo při DISABLE_RATE_LIMIT=1.
    Limity jde přepsat přes RATE_LIMIT_DEFAULTS (formát "1000 per hour;200 per 15 minutes").
  - CSRF kontrolu v before_request pro POST/PUT/PATCH/DELETE: session token
    `_csrf_token` vs. `csrf_token` ve form/JSON body nebo hlavička X-CSRFToken.

Endpointy blueprintů z EXEMPT_PREFIXES (např. vlastni_music_bp.*) přeskakují
CSRF i rate limiting. Do šablon je k dispozici Jinja global `csrf_token()`.
"""
