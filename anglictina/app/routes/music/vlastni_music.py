"""Vlastní hudba (blueprint `vlastni_music_bp`).

Uživatel si vybere vlastní píseň (YouTube embed), aplikace vytvoří cvičení
z jejího textu. Jediný modul používající OpenAI SDK přímo (OPENAI_API_KEY).
Pozor: endpointy tohoto blueprintu mají výjimku z CSRF/rate-limitu
(security_ext.EXEMPT_PREFIXES) a uvolněnou CSP (main.py) kvůli YouTube.
"""
