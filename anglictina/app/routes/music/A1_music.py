"""Hudební cvičení z předpřipravených písní (blueprint `exercises`).

Doplňování slov do textů písní: lyrics se čtou z JSON/LRC souborů ve
static/music/lyrics_json, překlady řádků zajišťuje DeepL (DEEPL_API_KEY)
s cache přes music_cache.py. Po dokončení lekce se přidává XP a streak.
"""
...
LYRICS_JSON_DIR = os.path.join(APP_DIR, 'static/music/lyrics_json')
